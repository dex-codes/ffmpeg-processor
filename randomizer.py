...
def main():
    ...
    # Example with optimal settings (5 categories, 20 items per category)
    categories = [f"cat{i}" for i in range(1, 6)]
    
    generator = ItemSequenceGenerator(
        categories=categories,
        items_per_category=20,  # Optimal for 5 categories
        variations_per_item=7,
        min_spacing=2
    )
    
    try:
        print("Generating sequence with optimal settings (5 cats, 20 items/cat)...")
        sequence = generator.generate_sequence(
            target_categories=categories,
            allowed_variations=[1, 3],
            sequence_length=150
        ...
        print(f"\n✓ Generated {len(sequence)} items successfully!")
        print(f"Distribution: {dict(category_counts)}")
        print(f"Violations: {len(violations)}")
        print(f"Items used per category: {list(category_counts.values())}")
        print(f"Items available per category: {20 * 2} (20 items × 2 variations)")
